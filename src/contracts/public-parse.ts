...
export interface PublicCombatTimeline {
  schema_version: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9; source: "single_report" | "reconciled_canonical_spine";
  ...
  participant_tracks: Array<{ actor_id: string; character_id: string | null; observed_character_key: string | null;
    ...
    skill_observation?: PublicTimelineSkillObservation }>;
  ...
  status_spans?: PublicTimelineStatusSpan[];
  ...
  omitted: { participant_tracks: number; series_points: number; death_markers: number; loadout_markers: number; skill_uses?: number; hostile_casts?: number; status_spans?: number; rdps_influence_spans: number; rate_clock_points?: number };
}
export interface PublicTimelineSkillObservation {
  coverage: "unavailable" | "partial" | "complete";
  evidence: Array<"exact_local_outbound" | "reconciled_local_vantage">;
}
...
export interface PublicTimelineStatusSpan {
  target_actor_id: string; source_actor_id?: string; effect_id: string; instance_id: string;
  start_micros: number; end_micros: number; terminal_state: "consumed" | "removed";
  evidence: PublicTimelineStatusSpanEvidence[]; omitted_evidence: number;
}
export interface PublicTimelineStatusSpanEvidence {
  source_report_id: string; applied_event_sequence: number; terminal_event_sequence: number;
  applied_game_time_millis?: number; terminal_game_time_millis?: number;
}
...
export function isPublicRunReconciliation(value: unknown): value is PublicRunReconciliation {
  ...
  if (!((value.schema_version === 15 && (timelineSchema === 1 || timelineSchema === 2)) ||
      ...
      (value.schema_version === 21 && timelineSchema === 8) ||
      (value.schema_version === 22 && timelineSchema === 9))) return false;
...
function isPublicRun(value: unknown, reportId: string, timelineSchema: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9): boolean {
...
function isParticipant(value: unknown, timelineSchema: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9): boolean {
...
function isTimeline(value: unknown, participants: readonly unknown[], requireResolvedTracks: boolean): value is PublicCombatTimeline {
  return isRecord(value) && (value.schema_version === 1 || value.schema_version === 2 || value.schema_version === 3 || value.schema_version === 4 || value.schema_version === 5 || value.schema_version === 6 || value.schema_version === 7 || value.schema_version === 8 || value.schema_version === 9) && (value.source === "single_report" || value.source === "reconciled_canonical_spine") &&
    ...
    value.participant_tracks.every((track) => isTimelineTrack(
      track, participants, value.duration_micros, requireResolvedTracks, value.schema_version, value.source,
    )) &&
    ...
    isTimelineHostileCasts(value, value.duration_micros) &&
    isTimelineStatusSpans(value, value.duration_micros) &&
...
function isSeriesPoint(value: unknown, timelineSchema: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9): boolean {
...
function isRateClock(value: Record<string, any>, durationMicros: number): boolean {
  if (value.schema_version !== 3 && value.schema_version !== 4 && value.schema_version !== 5 && value.schema_version !== 6 && value.schema_version !== 7 && value.schema_version !== 8 && value.schema_version !== 9) {
...
function isTimelineTrack(value: unknown, participants: readonly unknown[], durationMicros: number, requireResolved: boolean,
  timelineSchema: number, timelineSource: PublicCombatTimeline["source"]): boolean {
...
function isTimelineSkillObservation(value: unknown, timelineSource: PublicCombatTimeline["source"]): value is PublicTimelineSkillObservation {
  if (!isRecord(value) || !["unavailable", "partial", "complete"].includes(String(value.coverage)) ||
      !Array.isArray(value.evidence) || value.evidence.length > 2 ||
      !value.evidence.every((item: unknown) => item === "exact_local_outbound" || item === "reconciled_local_vantage") ||
      new Set(value.evidence).size !== value.evidence.length) return false;
  if (value.coverage === "unavailable") return value.evidence.length === 0;
  const requiredEvidence = timelineSource === "single_report" ? "exact_local_outbound" : "reconciled_local_vantage";
  return value.evidence.length === 1 && value.evidence[0] === requiredEvidence;
}
...
function isTimelineHostileCasts(value: Record<string, any>, durationMicros: number): boolean {
  ...
  for (const cast of value.hostile_casts) {
    if (!isRecord(cast) || !isBoundedIdentifierText(cast.source_actor_id) || !hostileSources.has(cast.source_actor_id) ||
        ...
        (cast.action_instance_id !== undefined && !isBoundedIdentifierText(cast.action_instance_id)) ||
        cast.state !== "started" || !Array.isArray(cast.evidence) || cast.evidence.length < 1 || cast.evidence.length > 8 ||
        !cast.evidence.every((evidence: unknown) => isTimelineCastEvidence(evidence, value.contributing_report_ids)) ||
        !isNonNegativeInteger(cast.omitted_evidence)) return false;
    ...
    const evidenceKeys = cast.evidence.map((evidence: PublicTimelineSkillUseEvidence) =>
      JSON.stringify([evidence.source_report_id, evidence.event_sequence, evidence.game_time_millis ?? null, evidence.kind]));
    if (!unique(evidenceKeys)) return false;
  }
  return true;
}
function isTimelineCastEvidence(value: unknown, reports: readonly unknown[]): value is PublicTimelineSkillUseEvidence {
  return isRecord(value) && typeof value.source_report_id === "string" && reports.includes(value.source_report_id) &&
    isNonNegativeInteger(value.event_sequence) &&
    (value.game_time_millis === undefined || Number.isSafeInteger(value.game_time_millis)) &&
    value.kind === "exact_wire_cast_start";
}
function isTimelineStatusSpans(value: Record<string, any>, durationMicros: number): boolean {
  if (value.schema_version < 8) {
    return value.status_spans === undefined && (!isRecord(value.omitted) || value.omitted.status_spans === undefined);
  }
  if (!Array.isArray(value.status_spans) || value.status_spans.length > 65_536 ||
      !isRecord(value.omitted) || !isNonNegativeInteger(value.omitted.status_spans)) return false;
  const participants = new Set(value.participant_tracks.map((track: unknown) => isRecord(track) ? track.actor_id : undefined));
  const perTarget = new Map<string, number>();
  const keys = new Set<string>();
  for (const span of value.status_spans) {
    if (!isRecord(span) || !isBoundedIdentifierText(span.target_actor_id) || !participants.has(span.target_actor_id) ||
        (span.source_actor_id !== undefined && (!isBoundedIdentifierText(span.source_actor_id) || !participants.has(span.source_actor_id))) ||
        !isBoundedIdentifierText(span.effect_id) || !isBoundedIdentifierText(span.instance_id) ||
        !isNonNegativeInteger(span.start_micros) || !isNonNegativeInteger(span.end_micros) ||
        span.start_micros > span.end_micros || span.end_micros > durationMicros ||
        (span.terminal_state !== "consumed" && span.terminal_state !== "removed") ||
        !Array.isArray(span.evidence) || span.evidence.length < 1 || span.evidence.length > 8 ||
        !isNonNegativeInteger(span.omitted_evidence)) return false;
    const count = (perTarget.get(span.target_actor_id) ?? 0) + 1;
    if (count > 16_384) return false;
    perTarget.set(span.target_actor_id, count);
    ...
    for (const evidence of span.evidence) {
      if (!isRecord(evidence) || typeof evidence.source_report_id !== "string" ||
          !value.contributing_report_ids.includes(evidence.source_report_id) ||
          !isNonNegativeInteger(evidence.applied_event_sequence) ||
          !isNonNegativeInteger(evidence.terminal_event_sequence) ||
...
function isOmitted(value: unknown): boolean {
  return isRecord(value) && ["participant_tracks", "series_points", "death_markers", "loadout_markers", "rdps_influence_spans"]
    ...
    (value.hostile_casts === undefined || isNonNegativeInteger(value.hostile_casts)) &&
    (value.status_spans === undefined || isNonNegativeInteger(value.status_spans));
}
